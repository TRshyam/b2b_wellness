  ...
  return (
    <div className="mb-8">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-helper text-slate-500 dark:text-slate-400">
          Personalized Corporate Intervention
        </span>
      </div>

      {/* Hero Recommendation - Soft Glass Accent Card */}
      <div className="accent-glass p-6 md:p-8 rounded-3xl relative overflow-hidden border border-emerald-500/30 shadow-2xl bg-gradient-to-br from-slate-900/90 via-slate-900/80 to-emerald-950/20 text-white">
        <div className="absolute top-0 right-0 w-64 h-64 bg-emerald-500/10 rounded-full blur-3xl pointer-events-none"></div>

        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
          <div className="space-y-3 max-w-2xl">
            <div className="flex items-center gap-2.5">
              <span className="text-xs font-bold uppercase tracking-wider px-3 py-1 rounded-lg bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
                Priority #{topRec.priority} Priority Intervention
              </span>
              <span className="text-helper text-slate-300 font-medium capitalize">
                {topRec.category.replace('_', ' ')}
              </span>
            </div>

            <h3 className="text-card-title md:text-2xl font-extrabold text-white">
              {topRec.action}
            </h3>

            <p className="text-body text-slate-200 leading-relaxed">
              <span className="font-semibold text-emerald-400">Why we're recommending this: </span>
              "{topRec.justification}"
            </p>

            <div className="pt-1 flex flex-wrap items-center gap-3">
              <div className="inline-flex items-center gap-2 text-helper font-semibold text-cyan-300 bg-cyan-500/10 px-3.5 py-1.5 rounded-xl border border-cyan-500/20">
          ...
          <div className="lg:w-72 shrink-0 flex flex-col justify-center space-y-3">
            <button className="w-full py-3.5 px-5 rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-400 hover:from-emerald-400 hover:to-teal-300 text-slate-950 font-extrabold text-body transition-all duration-200 flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/25 group">
              <span>Book Recommended Session</span>
              <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
            </button>
            <span className="text-xs text-center text-slate-300 block">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-amber-600 dark:text-amber-300 uppercase tracking-wider">Priority #{secondaryRec.priority}</span>
